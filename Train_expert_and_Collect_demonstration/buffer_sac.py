...
class Buffer(SerializedBuffer):
    def __init__(self, buffer_size, state_shape, action_shape, device):
        ...
        self.device = device
        # print("State shapeee",*state_shape)
        # print("Action shapeee",*action_shape)
        ac = (1,*action_shape)
        # ac = *action_shape
        # print("Action shapeee",*ac)
        self.states = torch.empty(
            (buffer_size, *state_shape), dtype=torch.float, device=device)
        self.actions = torch.empty(
            (buffer_size, *ac), dtype=torch.float, device=device)
    ...
    def append(self, state, action, reward, done, next_state):
        ...
        # print(self.actions[self._p].shape)
        # print(action.shape)
